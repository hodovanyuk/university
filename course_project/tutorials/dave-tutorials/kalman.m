...
% deviation of 2.
x=zeros(2,N); % x will be the initialized variable for where the ninja
% thinks he hears the quail.
...
Po=Po/sum(sum(Po)); % Each value is now 1/(number of states), so it all
% sums to one.
...
[a,b]=find(Po==max(max(Po)));  % Pull out the indices at which Po achieves
% its max to start.
...
for (n=2:length(x));
    ...
    for (i=1:length(Pr)) 
       for (j=1:length(Pr))
           me=[Sa(i);Sb(j)];
           m(i,j) = 1/sqrt((2*pi)^2*det(K)) * exp(-(x(:,n)-me)'*...
               inv(K)*(x(:,n)-me)/2); %Compute likelihood            
